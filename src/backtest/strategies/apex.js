...
function signalApex(ctx) {
  ...
  checks.push(vol252 > 0 && vol20 < vol252 * 1.4);
  ...
  const regimeThrottle = Math.max(0, Math.min(1, (score - 0.4) / 0.4));

  const grossExposure = Math.max(0.25, Math.min(1, volScale * regimeThrottle));
